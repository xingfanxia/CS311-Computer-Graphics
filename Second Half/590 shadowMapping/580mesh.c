...
struct meshGLMesh {
	GLuint triNum, vertNum, attrNum,vaoNum, attrDim;
	GLuint *attrDims;
	GLuint buffers[2];
	GLuint *vaos;
};
...
/* This function initializes the VAO at that index in the meshGL's array of VAOs, 
so that the VAO can render using those locations. */
void meshGLVAOInitialize(meshGLMesh *meshGL, GLuint index, GLint attrLocs[]){

	glBindVertexArray(meshGL->vaos[index]);
	/* Tell the VAO about the attribute arrays and hook into 
	the vertex shader. Moving these into init stage greatly reduces rendering time */
	glBindBuffer(GL_ARRAY_BUFFER, meshGL->buffers[0]);
	int offset = 0;
	...
	/* Tell the VAO about triangles to draw */
	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, meshGL->buffers[1]);
	/*Unbind the VAO*/
	glBindVertexArray(0);
}
